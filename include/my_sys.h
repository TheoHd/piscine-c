...
extern char* my_strdup_with_length(
    const byte* from, uint length,
    myf MyFlags
);
...
#endif                    /* #ifndef errno */
extern char NEAR
errbuff[NRERRBUFFS][ERRMSGSIZE];
extern char* home_dir;            /* Home directory for user */
extern const char* my_progname;        /* program-name (printed in errors) */
extern char NEAR
curr_dir[];        /* Current directory for user */
extern int (* error_handler_hook)(uint my_err, const char* str, myf MyFlags);
extern int (* fatal_error_handler_hook)(
    uint my_err, const char* str,
    myf MyFlags
);
...
extern int NEAR
my_umask,        /* Default creation mask  */
NEAR my_umask_dir,
    NEAR
...
typedef int (* Process_option_func)(
    void* ctx, const char* group_name,
    const char* option
);
...
extern File my_register_filename(
    File fd, const char* FileName,
    enum file_type type_of_file,
    uint error_message_number, myf MyFlags
);
extern File my_create(
...
extern File my_create_with_symlink(
    const char* linkname, const char* filename,
...
extern uint my_pread(
    File Filedes, byte* Buffer, uint Count, my_off_t offset,
    myf MyFlags
);
extern int my_rename(const char* from, const char* to, myf MyFlags);
extern my_off_t my_seek(File fd, my_off_t pos, int whence, myf MyFlags);
extern my_off_t my_tell(File fd, myf MyFlags);
extern uint my_write(
    File Filedes, const byte* Buffer, uint Count,
    myf MyFlags
);
extern uint my_pwrite(
    File Filedes, const byte* Buffer, uint Count,
    my_off_t offset, myf MyFlags
);
extern uint my_fread(FILE* stream, byte* Buffer, uint Count, myf MyFlags);
extern uint my_fwrite(
    FILE* stream, const byte* Buffer, uint Count,
    myf MyFlags
);
extern my_off_t my_fseek(FILE* stream, my_off_t pos, int whence, myf MyFlags);
extern my_off_t my_ftell(FILE* stream, myf MyFlags);
extern gptr _mymalloc(
    uint uSize, const char* sFile,
    uint uLine, myf MyFlag
);
extern gptr _myrealloc(
    gptr pPtr, uint uSize, const char* sFile,
    uint uLine, myf MyFlag
);
extern gptr my_multi_malloc
_VARARGS((myf
MyFlags, ...));
extern void _myfree(gptr pPtr, const char* sFile, uint uLine, myf MyFlag);
extern int _sanity(const char* sFile, unsigned int uLine);
extern gptr _my_memdup(
    const byte* from, uint length,
    const char* sFile, uint uLine, myf MyFlag
);
extern my_string _my_strdup(
    const char* from, const char* sFile, uint uLine,
    myf MyFlag
);
extern char* _my_strdup_with_length(
    const byte* from, uint length,
...
extern my_string fn_format(
    my_string to, const char* name, const char* dir,
...
extern my_string my_path(
    my_string to, const char* progname,
    const char* own_pathname_part
);
extern my_string my_load_path(
    my_string to, const char* path,
...
extern int init_record_cache(
    RECORD_CACHE* info, uint cachesize, File file,
    uint reclength, enum cache_type type,
    pbool use_async_io
);
extern int read_cache_record(RECORD_CACHE* info, byte* to);
extern int end_record_cache(RECORD_CACHE* info);
extern int write_cache_record(
...
extern void radixsort_for_str_ptr(
    uchar* base[], uint number_of_elements,
    size_s size_of_element, uchar* buffer[]
);
extern qsort_t qsort2(
    void* base_ptr, size_t total_elems, size_t size,
    qsort2_cmp cmp, void* cmp_argument
);
extern qsort2_cmp get_ptr_compare(uint);
void my_store_ptr(byte* buff, uint pack_length, my_off_t pos);
my_off_t my_get_ptr(byte* ptr, uint pack_length);
extern int init_io_cache(
    IO_CACHE* info, File file, uint cachesize,
    enum cache_type type, my_off_t seek_offset,
    pbool use_async_io, myf cache_myflags
);
extern my_bool reinit_io_cache(
...
extern int my_block_write(
    IO_CACHE* info, const byte* Buffer,
...
extern my_bool open_cached_file(
...
File create_temp_file(
    char* to, const char* dir, const char* pfx,
...
extern my_bool init_dynamic_array(
...
extern my_bool init_dynamic_string(
    DYNAMIC_STRING* str, const char* init_str,
    uint init_alloc, uint alloc_increment
);
extern my_bool dynstr_append(DYNAMIC_STRING* str, const char* append);
my_bool dynstr_append_mem(
    DYNAMIC_STRING* str, const char* append,
    uint length
);
...
extern void init_alloc_root(
    MEM_ROOT* mem_root, uint block_size,
    uint pre_alloc_size
);
...
extern void reset_root_defaults(
    MEM_ROOT* mem_root, uint block_size,
    uint prealloc_size
);
...
extern int get_defaults_options(
    int argc, char** argv,
    char** defaults, char** extra_defaults,
    char** group_suffix
);
extern int load_defaults(
    const char* conf_file, const char** groups,
    int* argc, char*** argv
);
extern int modify_defaults_file(
    const char* file_location, const char* option,
    const char* option_value,
    const char* section_name, int remove_option
);
extern int my_search_option_files(
    const char* conf_file, int* argc,
...
extern CHARSET_INFO* get_charset_by_csname(
    const char* cs_name,
...
extern ulong escape_string_for_mysql(
...
#endif
extern ulong escape_quotes_for_mysql(
    CHARSET_INFO* charset_info,
    char* to, ulong to_length,
    const char* from, ulong length
);
